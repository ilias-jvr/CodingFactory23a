...
public class CitiesIOApp {
    ...
    public static void main(String[] args) {
        ...
        try (BufferedReader bf = new BufferedReader(new FileReader("C:/tmp/cities.txt"))) {
            while ((line = bf.readLine()) != null) { //ελέγχει το αριστερό κομμάτι της ισότητας line (ιδιωματισμός γλώσσας)
                cities = line.split(" +"); //το + είναι ποσοδείκτης ένα ή περισσότερο κενά
                switch (cities[0]) {
                    case "Greece":
    ...
    public static void print(PrintStream ps, String[] tokens) {
        for (int i = 1; i < tokens.length; i++){
            ps.print(tokens[i] + " ");
//            print(ps, tokens[i] + " "); θα αφήνει γραμμή
        }
    }
